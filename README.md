...
- Двухшаговый импорт: загрузка файла → сопоставление колонок с полями шаблона
...
2) На шаге сопоставления выберите, какие колонки Excel соответствуют полям шаблона (обязательные поля подсвечены).
3) После запуска импорта статус и статистика (успешно/всего/ошибки) доступны в разделе задач.
